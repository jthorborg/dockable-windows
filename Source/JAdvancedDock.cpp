	...
	*/
	...
	public:
		...
		void resized() override
		{
			...
			buttons[AdvancedDockPlaces::top]->setBounds(topArea.toType<int>());

			midArea = area.removeFromTop(h3);

			auto midAreaChop = midArea;

			buttons[AdvancedDockPlaces::left]->setBounds(midAreaChop.removeFromLeft(w3).toType<int>());
			centreArea = midAreaChop.removeFromLeft(w3);
			buttons[AdvancedDockPlaces::centre]->setBounds(centreArea.toType<int>());
			buttons[AdvancedDockPlaces::right]->setBounds(midAreaChop.toType<int>());

			bottomArea = area.translated(h3, 0.0f).withWidth(h3);
			buttons[AdvancedDockPlaces::bottom]->setBounds(bottomArea.toType<int>());
		}
